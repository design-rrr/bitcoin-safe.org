...
1. **COLDCARD 以外の署名デバイス**（[対応デバイスを見る]({{< ref "library/supported-hardware-signers" >}})）で、Bitcoin-Safe のセットアップウィザードを使って完全に新しいウォレットを作成します。[シングルシグウォレット]({{< ref "library/setup-singlesig-wallet" >}})または[マルチシグウォレット]({{< ref "library/setup-multisig-wallet" >}})を選び、古いシードは再利用しないでください。
...
2. 移行トランザクションに署名しますが、**通常の方法ではブロードキャストしないでください**。署名済み raw transaction をコピーし、[MARA Slipstream](https://slipstream.mara.com/) から直接送信することで、古いウォレットの公開鍵が公開 mempool に露出する時間を短縮します。
