...
1. Na **urządzeniu podpisującym innym niż COLDCARD** ([zobacz obsługiwane urządzenia]({{< ref "library/supported-hardware-signers" >}})) utwórz całkowicie nowy portfel za pomocą kreatora konfiguracji Bitcoin-Safe: wybierz [portfel z jednym podpisem]({{< ref "library/setup-singlesig-wallet" >}}) albo [portfel multisig]({{< ref "library/setup-multisig-wallet" >}}). Nie używaj ponownie starego seeda.
...
2. Podpisz transakcję migracyjną, ale **nie rozgłaszaj jej w zwykły sposób**. Skopiuj podpisaną surową transakcję i prześlij ją bezpośrednio przez [MARA Slipstream](https://slipstream.mara.com/), aby skrócić czas ujawnienia kluczy publicznych starego portfela w publicznym mempoolu.
