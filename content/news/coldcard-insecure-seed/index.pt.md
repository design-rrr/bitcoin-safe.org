...
1. Num **dispositivo de assinatura que não seja COLDCARD** ([ver dispositivos suportados]({{< ref "library/supported-hardware-signers" >}})), cria uma carteira totalmente nova com o assistente de configuração do Bitcoin-Safe: escolhe uma [carteira de assinatura única]({{< ref "library/setup-singlesig-wallet" >}}) ou uma [carteira multifirma]({{< ref "library/setup-multisig-wallet" >}}). Não reutilizes a seed antiga.
...
2. Assina a transação de migração, mas **não a transmitas normalmente**. Copia a transação bruta assinada e envia-a diretamente através do [MARA Slipstream](https://slipstream.mara.com/) para reduzir o tempo durante o qual as chaves públicas da carteira antiga ficam expostas na mempool pública.
